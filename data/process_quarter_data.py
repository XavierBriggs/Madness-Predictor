#!/usr/bin/env python3
"""Compute per-team, per-quarter advanced basketball metrics from raw game logs."""
...
def compute_team_per_quarter_matrix(csv_file):
    """Build a feature matrix of 14 per-possession metrics for each team-quarter."""
    ...
    for prefix in ["W", "L"]:
        ...
        df[f"{prefix}OREB%"] = df[f"{prefix}OR"] / (df[f"{prefix}OR"] + df[f"{opp}DR"])
        df[f"{prefix}DREB%"] = df[f"{prefix}DR"] / (df[f"{prefix}DR"] + df[f"{opp}OR"])
...
if __name__ == "__main__":
    csv_file = os.path.join(RAW_DIR, "MRegularSeasonDetailedResults_with_poss.csv")
    output_path = os.path.join(PROCESSED_DIR, "teams.csv")
    team_matrix = compute_team_per_quarter_matrix(csv_file)
    team_matrix.to_csv(output_path, index=False)
    print("Done")
